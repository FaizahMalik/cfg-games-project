...
br_words = brown.tagged_words(tagset='universal') # a list of tuples (word, tag): [('The', 'DET'), ('Fulton',
# 'NOUN'), ...]

# Lemmatize means to use the dictionary form of the word, e.g. cats - cat, knew - know.
lemmatizer = WordNetLemmatizer()

# Stop words are very common words that do not add much to the meaning (articles, pronouns, prepositions,
# auxiliary verbs). We don't want the users to guess such words in our game so we need to filter them from the Brown
# corpus.
...
filtered_words = [(lemmatizer.lemmatize(word.lower(), pos = wn_tags[tag]), tag)  # a list of tuples: lemmatized
                  # lowercase word and its POS tag
...
# FreqDist (frequency distribution) is used to find the frequency of words within a text. It returns a dictionary of
...
final_wordlist = [tup for tup, count in freq_words] # a list of tuples: word, tag. Since it is already sorted, we do
# not need the count anymore. [('say', 'VERB'), ('would', 'VERB'), ('make', 'VERB')...]
...
# For other tasks, we can play with the word length, part of speech, is_unique_chars function, how common the word is
# out of 2000 most common and so on, combining these features if we want to make it more difficult.
